...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.target_vocab_projection = nn.Linear(self.hidden_size, len(self.vocab.tgt),bias=False)# second dim - size of target vocab 
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        Y = nn.utils.rnn.pack_padded_sequence(X, source_lengths)
        enc_hids, (last_hidden, last_cell) = self.encoder(Y)
        enc_hiddens_sec = nn.utils.rnn.pad_packed_sequence(enc_hids)[0]
        enc_hiddens = enc_hiddens_sec.permute(1,0,2)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        #L = tgt_len
        Y_split = torch.split(Y,1,0)

        for t in range(tgt_len): 
            
            Ybar_t = torch.cat((torch.squeeze(Y_split[t], dim =0),o_prev),1)
            dec_state, o_prev, e_t = self.step(Ybar_t, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks) 
            combined_outputs.append(o_prev)
        
        combined_outputs = torch.stack(combined_outputs, dim=0) #check dimension of combined_outputs
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        ###     Tensor Squeeze:
        ###         https://pytorch.org/docs/stable/torch.html#torch.squeeze
        dec_state = self.decoder(Ybar_t, dec_state) #Ybar_t = (b,e+h), dec_state = tuple of (b,h)
        dec_unsqueezed = torch.unsqueeze(dec_state[0], 2)
        e_t = torch.squeeze(torch.bmm(enc_hiddens_proj, dec_unsqueezed),2)
        ### END YOUR CODE
        ...
        alpha_t_unsqueezed = torch.unsqueeze(alpha_t,1)
        a_t = torch.squeeze(torch.bmm(alpha_t_unsqueezed, enc_hiddens),1)
